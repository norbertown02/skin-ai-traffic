...
export default function EntityDeep({data,type}){
 ...
 <div className="analysis-highlight-row"><Surface eyebrow="MAIOR VERBA" title="Maior concentração"><strong className="analysis-big">{biggest?.name||'—'}</strong><p className="muted-copy">Gasto {money(biggest?.spend)} · ROAS {num(biggest?.roas)}</p></Surface><Surface eyebrow="MELHOR RETORNO" title="Melhor item com amostra"><strong className="analysis-big">{best?.name||'—'}</strong><p className="muted-copy">ROAS {num(best?.roas)} · CAC {money(best?.cac)}</p></Surface><Surface eyebrow="COBERTURA" title="Itens com entrega"><strong className="analysis-big">{integer(rows.length)}</strong><p className="muted-copy">Itens com dados no período atual.</p></Surface></div>
 ...
 <div className="deep-two"><HorizontalBars title="Gasto por item" rows={rows.map(x=>({key:x.name||'—',spend:x.spend}))} valueKey="spend" formatValue={money}/><HorizontalBars title="ROAS por item" rows={rows.filter(x=>n(x.spend)>=20).map(x=>({key:x.name||'—',roas:x.roas}))} valueKey="roas" formatValue={num}/></div>
